...
static volatile uint8_t spi_d[8*BYTES_PER_LED*NUMBER_LEDS*BITS];
...
ws2811_ret_E ws2811_trigger(void) {
	...
	for(uint32_t i = 0;i < BYTES_PER_LED*NUMBER_LEDS;i++) {
		...
		for(uint8_t j = 0; j < 8; j++) {
			...
			if (bit == 0) {
				spi_d[i*8*BITS + j*BITS + 0] = 0;
				spi_d[i*8*BITS + j*BITS + 1] = 0;
				spi_d[i*8*BITS + j*BITS + 2] = 0;
				spi_d[i*8*BITS + j*BITS + 3] = 1;
			} else {
				spi_d[i*8*BITS + j*BITS + 0] = 0;
				spi_d[i*8*BITS + j*BITS + 1] = 0;
				spi_d[i*8*BITS + j*BITS + 2] = 1;
				spi_d[i*8*BITS + j*BITS + 3] = 1;
			}
			mask = mask >> 1;
		}
	...
	uint32_t byte_i = 0;
	uint8_t bit_i = 7;
	for(uint32_t i = 0;i < 8*BYTES_PER_LED*NUMBER_LEDS*BITS;i++) {
		output_buff[byte_i] = output_buff[byte_i] | (spi_d[i] << bit_i);
		if (bit_i == 0) {
			bit_i = 8;
			byte_i++;
		}
